...
1. Yeni bir klasör aç, planlama `.md` dosyalarını `/docs` altına koy (PROJECT, CONTENT-STRATEGY,
   SITEMAP, DESIGN-SYSTEM, TECH-STACK, SKILLS-MCP, PLAN, 3D-FEATURE, BUILD-TASKS, QA-CHECKLIST, SETUP, BLOG, HERO).
...
- docs/PROJECT.md, docs/CONTENT-STRATEGY.md, docs/SITEMAP.md, docs/DESIGN-SYSTEM.md,
  ...
  docs/BUILD-TASKS.md, docs/QA-CHECKLIST.md, docs/SETUP.md, docs/BLOG.md, docs/HERO.md, docs/MOTION.md
...
HAREKET (MOTION.md): doz = abartısız premium + 1-2 imza anı; Motion+Lenis Faz 4'te eklenir, reduced-motion tam destek, Lighthouse ≥95 korunur.
...
Header(sticky, TR/EN, WhatsApp) → Hero → Hikâye(+3D karusel) → Ürünler(+3D et kâşifi)
→ İmza lezzetler → Mangal & davet → Nasıl sipariş(3 adım) → Instagram → Konum/iletişim → Footer.
Kalıcı: yüzen WhatsApp + mobil sabit alt CTA bar. Ayrıca /blog rotaları.

HERO — SPLIT-SCREEN (HERO.md — birebir):
- İki panel: SOL ~%52 krem, SAĞ ~%48 kömür. Üstte iki paneli kesen ince bar (logo+nav+TR/EN+WhatsApp).
- DEV knockout lockup: "DENİZ" sol-üst (harf içleri köz/ateş dokusu, background-clip:text, hafif parlar),
  "ET" sağ-alt krem dolu. Marka adı split okunur. H1 (semantik) = "Deniz Et · Kasap Orhan".
- SOL "about" bloğu: üst etiket → çok sessiz miras satırı "Üç nesildir / Three generations"
  (hero.heritage; Bonny Light/Thin, ink-soft, küçük) → tagline "Kırk yıldır aynı el, aynı söz." → alt metin →
  [WhatsApp'tan Sipariş Ver] + Instagram + adres. (NOT: ayrı "güven şeridi" bölümü YOK — kaldırıldı.)
- SAĞ panel: 3D hero objesi = közün üstünde süzülen et (Tripo gelene dek çarpıcı still), ince yörünge +
  kıvılcım aksanlar. (İnteraktif 10-parça kâşifi burada DEĞİL; o Ürünler'de.)
- Statik + hafif hareket (köz parıltısı, yavaş süzülme); reduced-motion'da donar. Mobilde paneller yığılır.
- Hikâye: çiğ → köz → sofra. Referans ilkesi alındı, klon değil; görsel dil tamamen kasap/ateş.
